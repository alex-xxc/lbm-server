...
export class DictEntity {
    ...
    @Column({ type: 'varchar', name: 'dict_name', length: 255, comment: '字典名称' })
    public dictName: string

    @ApiProperty({ type: String, description: '字典类型' })
    @Column({ type: 'varchar', name: 'dict_type', length: 255, comment: '字典类型' })
    public dictType: string

    @ApiProperty({ type: String, description: '字典描述' })
    @Column({ type: 'varchar', name: 'description', length: 400, comment: '字典描述' })
    public description: string

    @ApiProperty({ type: String, description: '字典内容' })
    @Column({ type: 'varchar', name: 'content', length: 600, comment: '字典内容' })
