...
import { LoginlogService } from 'src/system/loginlog/loginlog.service'
...
export class UserService {
  ...
  /** 创建用户 */
  async create(dto: CreateUserDto, account: string, ip: string): Promise<ResultData> {
   ...
   */
  async login(account: string, password: string, ip: string, loginSystem: string, browserData: string): Promise<ResultData> {
    let user = null
    ...
    let loginData = {
      ...
      loginIp: ip,
      loginAddress: `${ipAddress.province}-${ipAddress.city}`,
      loginBrowser: browserData,
      loginSystem: loginSystem,
      status: 0
    }
    console.log(loginData, '用户登录地址>>>>>：', `${ipAddress.province}-${ipAddress.city}`);
    if (!user) {
      // 登录失败，插入登录日志
      loginData.status = 0;
      this.loginlogService.create(loginData);
   ...
   * 清除redis
   */
  async loginOut(id: string): Promise<ResultData> {
    // 根据用户id，查询有没有该用户
    const user = await this.findOneById(id);
    if (!user) return ResultData.fail(AppHttpCode.USER_NOT_FOUND, '该用户不存在或已删除');
    if (user.type !== 0) {
      await this.redisService.del(getRedisKey(RedisKeyPrefix.USER_PERM, user.id));
    }
    await this.redisService.del(getRedisKey(RedisKeyPrefix.USER_INFO, user.id));
    await this.redisService.del(getRedisKey(RedisKeyPrefix.USER_MENU, user.id));
    await this.redisService.del(getRedisKey(RedisKeyPrefix.USER_ROLE, user.id));
   ...
   * 批量导入用户
   */
  async importUsers(file: Express.Multer.File, account: String, ip: String): Promise<ResultData> {
  ...
  /** 更新用户信息 */
  async update(dto: UpdateUserDto, currUser: UserEntity, account: String, ip: String): Promise<ResultData> {
    ...
    this.operalogService.create({
      systemMenu: '用户管理',
      operaModule: `更新用户 -【${existing.account}】`,
      operaName: account,
      operaIp: ip,
      status: 1
    })
  ...
  async updateStatus(userId: string, status: 0 | 1, currUserId: string, account: String, ip: String): Promise<ResultData> {
    ...
    this.operalogService.create({
      systemMenu: '用户管理',
      operaModule: `更改用户状态 - ${status === 0 ? '禁用' : '启用'}【${existing.account}】`,
      operaName: account,
      operaIp: ip,
      status: 1
    })
   ...
   */
  async updatePassword(userId: string, password: string, reset: boolean, currUser: UserEntity, account: String, ip: String): Promise<ResultData> {
    ...
    // }
    if (existing.type === UserType.SUPER_ADMIN) {
      return ResultData.fail(AppHttpCode.USER_FORBIDDEN_UPDATE, '超级管理员账号密码禁止修改！')
    }
    ...
    this.operalogService.create({
      systemMenu: '用户管理',
      operaModule: `重置用户密码 - 【${existing.account}】`,
      operaName: account,
      operaIp: ip,
      status: 1
    })
  ...
  async findList(dto: FindUserListDto): Promise<ResultData> {
    const { page, size, account, status, phoneNum, hasCurrRole = 0, deptId, hasCurrDept = 0 } = dto
    const where = {
      ...(status ? { status } : null),
